...
echo Compiling Asteria in %BUILD_CONFIG% configuration
pushd %BUILD_FOLDER%
...
if "%ML_SYMBOL_ARCHIVE%" NEQ "" (
    ...
    copy %SYMBOLS_FOLDER%\AsteriaDebuggingSymbols-%ARCH%-%VERSION%.zip %ML_SYMBOL_ARCHIVE%
    if !ERRORLEVEL! NEQ 0 goto Error
) else (
    if "%MUST_DEPLOY_SYMBOLS%"=="1" (
...
%WINDEPLOYQT_CMD% --dir %DEPLOY_FOLDER% --%BUILD_CONFIG% --qmldir %SOURCE_ROOT%\app\gui --no-opengl-sw --no-compiler-runtime --no-sql %WINDEPLOYQT_ARGS% %BUILD_FOLDER%\app\%BUILD_CONFIG%\Asteria.exe
if !ERRORLEVEL! NEQ 0 goto Error
...
if "%SIGN%"=="1" (
    ...
    set FILES_TO_SIGN=%BUILD_FOLDER%\app\%BUILD_CONFIG%\Asteria.exe
...
if "%ML_SYMBOL_STORE%" NEQ "" (
    ...
    symstore add /r /f %DEPLOY_FOLDER%\*.* /s %ML_SYMBOL_STORE% /t Asteria
    if !ERRORLEVEL! NEQ 0 goto Error
    symstore add /r /f %BUILD_FOLDER%\app\%BUILD_CONFIG%\Asteria.exe /s %ML_SYMBOL_STORE% /t Asteria
    if !ERRORLEVEL! NEQ 0 goto Error
)
...
copy %BUILD_FOLDER%\app\%BUILD_CONFIG%\Asteria.exe %DEPLOY_FOLDER%
if !ERRORLEVEL! NEQ 0 goto Error
...
7z a %INSTALLER_FOLDER%\AsteriaPortable-%ARCH%-%VERSION%.zip %DEPLOY_FOLDER%\*
if !ERRORLEVEL! NEQ 0 goto Error

echo Build successful for Asteria v%VERSION% %ARCH% binaries!
exit /b 0
