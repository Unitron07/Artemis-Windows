...
$scratch = Join-Path ([IO.Path]::GetTempPath()) ('artemis-repair-tests-' + [guid]::NewGuid().ToString('N'))
...
if ($env:ARTEMIS_REPAIR_TEST_MODE -eq 'error') { $global:LASTEXITCODE = 1; return }
if ($env:ARTEMIS_REPAIR_TEST_MODE -eq 'malformed') { 'unrecognized'; return }
'File Type: DLL'
'  Image has the following dependencies:'
'    KERNEL32.dll'
if ($env:ARTEMIS_REPAIR_TEST_MODE -eq 'normal') { '    VCRUNTIME140_1.dll' }
if ($env:ARTEMIS_REPAIR_TEST_MODE -eq 'delay') {
...
try {
    foreach ($mode in @('unused', 'normal', 'delay', 'error', 'malformed', 'native', 'absent', 'other-x64')) {
        ...
        try {
            $files = @{'Moonlight.exe' = 0xAA64; 'plugins/nested/Qt.dll' = 0xAA64}
        ...
        if (!$expectedFailure) {
            ...
            try { & (Join-Path $PSScriptRoot 'test-package-architecture.ps1') -PackagePath $package -Architecture arm64 -ReportPath (Join-Path $scratch "$mode-architecture.json") }
    ...
    $env:ARTEMIS_REPAIR_TEST_MODE = $savedMode
