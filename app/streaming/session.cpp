...
bool Session::validateLaunch(SDL_Window* testWindow)
{
    ...
    if (!SdlInputHandler::getUnmappedGamepads().isEmpty()) {
        emitLaunchWarning(tr("An attached gamepad has no mapping and won't be usable. Visit the Asteria help to resolve this."));
    }
...
#else
    std::string windowName = QString(m_Computer->name + " - Asteria").toStdString();
#endif
